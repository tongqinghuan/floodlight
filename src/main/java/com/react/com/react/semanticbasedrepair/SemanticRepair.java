    ...
    public static boolean semanticRepair(Instruction semantic_rules, Set<TwoTuple<String>> dof, int count) {
        ...
        for (TwoTuple<String> tuple : fixed_forward) {
            if (Network.edge_ports.contains(new Port(Integer.parseInt(tuple.second), current_switch_id))) {
                ...
                ConnectedSwitch temp = connected_switch.get(current_switch_id);
                connected_switch.clear();
                connected_switch.put(current_switch_id, temp);
            } else {
                return false;
            }
        }
        ...
        if (!towards.isEmpty()) {
            ...
            for (ConnectedSwitch sw : connected_switch.values()) {
                ...
                if (count <= maxhops) {
                    final_result = semanticRepair(ins, next_dof, ++count); //递归 缺update参数
                    if (final_result) {
...
//						new_dofs.add(new TwoTuple("forbid",current_switch_id));
	...
	public static void test(String[] args) {
		List<FourTuple> link=new ArrayList<FourTuple>();
		link.add(new FourTuple("A","B",3,3));
		link.add(new FourTuple("A","C",1,1));
		link.add(new FourTuple("B","D",1,1));
		link.add(new FourTuple("C","D",2,2));
		link.add(new FourTuple("B","E",2,2));
		link.add(new FourTuple("D","F",3,3));
		link.add(new FourTuple("E","G",1,1));
		link.add(new FourTuple("F","G",2,2));
//		Network.create_node(link);
//		Network.create_edge_port();
		...
		semantic_rules.put("B", new Instruction(scope1,constriant1,annotation1));
		...
		semantic_rules.put("D", new Instruction(scope2,constriant2,annotation2));
		...
		semantic_rules.put("F", new Instruction(scope3,constriant3,annotation3));
		...
		semantic_rules.put("G", new Instruction(scope4,constriant4,annotation4));
		...
		String exit_switch_id="G";
		SemanticRepair.semanticRepair(new Instruction(scope,constriant,annotation),art_dof);
		Collections.reverse(new_flow_semantic_rules);
		new_flow_semantic_rules.add(flow_semantic_rules.get("11.1.0.0/16").get(exit_switch_id));
		System.out.println(new_flow_semantic_rules);

		update_rules(flow_semantic_rules.get("11.1.0.0/16").values(),new_flow_semantic_rules);

		System.out.println(added_rules);
		System.out.println(deleted_rules);
	}
*/
}
